package org.sonatype.tycho.test.TYCHO359corruptedArtifactDownloads;
...
/**
 * This test uses the checked-in repository at
 * <code>repositories/e342_corrupt</code>, which was originally a copy of
 * <code>repositories/e342</code> but with all actual artifacts (i.e. jars)
...
public class RecoveryAfterCorruptArtifactDownloadsTest extends
	...
	public void testSuccessfulRebuildAfterCorruptedArtifactDownload() throws Exception {
		...
		Verifier verifier2 = getVerifier(
				"/TYCHO359corruptedArtifactDownloads", false);
	...
	/**
	 * Reads the given targetFile and converts all paths in locations into
	 * absolute file URIs.
	 * 
	 * Paths are expected to be relative to tycho-its directory.
	 * 
	 * TODO move this to a util class at org.sonatype.tycho.test.util and reuse
	 * in other tests.
	 * 
	 * @param targetFile
	 *            the target definition file to be processed
	 */
	private void convertLocationPathsToAbsoluteURIs(File targetFile)
		...
		for (Target.Location location : platform.getLocations()) {
			for (Repository repository : location.getRepositories()) {
				...
				repository.setLocation(file.getCanonicalFile().toURI()
						.toASCIIString());
			}
		}
