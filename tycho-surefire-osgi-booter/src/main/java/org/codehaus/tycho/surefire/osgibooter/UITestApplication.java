...
public class UITestApplication implements IApplication, ITestHarness {
	...
	public Object start(IApplicationContext context) throws Exception {
		...
		if (args == null)
			args = new String[0];
		IApplication application = getApplication(args);
	...
	public void runTests() {
		...
		fTestableObject.runTest(new Runnable() {
			public void run() {
				String[] args = (String[]) appContext.getArguments().get(IApplicationContext.APPLICATION_ARGS);
		        try {
					fTestRunnerResult = OsgiSurefireBooter.run(args);
				} catch (Exception e) {
	...
	private Object runApplication(IApplication application, Object args) throws Exception {
		fTestableObject = PlatformUI.getTestableObject();
		fTestableObject.setTestHarness(this);
		return application.start(appContext);
	}
